...
- **FR-006**: When every item can be reserved, the system MUST create an active reservation for each item, decrease available quantity by the held amount, increase reserved quantity by the same amount, and record a corresponding reservation stock-history movement for each item using the existing stock-history fields.
